...
export const OutcomeList: React.FC<OutcomeListProps> = ({ category, showDateSeparators = false }) => {
  ...
  const sortedOutcomeData = useMemo(() => {
    ...
    return dataToSort?.slice().sort((a, b) => new Date(b.created_at ?? "").getTime() - new Date(a.created_at ?? "").getTime()) ?? [];
  ...
  const handleLongPress = useCallback(
    (outcome: OutcomeData) => {
      ...
      Alert.alert("Eliminar gasto", "¿Está seguro de que quiere eliminar el gasto?", [
  ...
  const renderItem = useCallback(
    ({ item }: { item: OutcomeData | { date: string } }) => {
      if ("date" in item) {
        return (
