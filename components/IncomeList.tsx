...
export const IncomeList = () => {
  ...
  const sortedIncomeData = useMemo(() => {
    return incomeData?.slice().sort((a, b) => new Date(b.created_at ?? "").getTime() - new Date(a.created_at ?? "").getTime()) ?? [];
  }, [incomeData]);
  ...
  const handleLongPress = useCallback(
    (income: IncomeData) => {
      ...
      Alert.alert("Eliminar ingreso", "¿Está seguro de que quiere eliminar el ingreso?", [
  ...
  return (
    <View style={styles.container}>
      <FlatList data={sortedIncomeData} renderItem={renderItem} keyExtractor={keyExtractor} showsVerticalScrollIndicator={false} />
    </View>
  );
};
