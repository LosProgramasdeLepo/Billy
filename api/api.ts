...
export async function calculateDebts(billId: string): Promise<boolean> {
  try {
    ...
    for (const debt of debts) {
      const { error: insertError } = await supabase.from("BillDebts").insert({
        bill: billId,
        debtor: debt.debtor,
